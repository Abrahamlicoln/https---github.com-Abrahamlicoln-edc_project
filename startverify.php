...
$bonquet = filter_var(mysqli_real_escape_string($connection, $_POST['bonquet']), FILTER_SANITIZE_STRING);
$_SESSION['bonquet'] = $bonquet;
if (isset($_POST['buynow'])) {
    $account_number = filter_var(mysqli_real_escape_string($connection, $_POST['account_number']), FILTER_SANITIZE_STRING);
    ...
    if ($payment_method == "e-wallet") {
        ...
        if ($response->success == "true") {
            ...
            $_SESSION['name'] = $customer_name;
            echo '<script>
    window.location.href = "startverify2.php";
            </script>';
        } else {
            $_SESSION['erorrr'] = "Set";
        }
...
<head>
    ...
    <title>Startimes Subscription Verification| Synthesis Cafe</title>
    <!-- MDB icon -->
                                    ...
                                    <div class="row" style="margin-top:-10px;">
                                        <div class="col-12">
                                            <div class=" mb-3" style="margin-top:-40px;">
                                                ...
                                                <div class="mt-2 mb-5">
                                                    <center>
                                                        <h5>You are about to do Startimes Subscription for <?php echo "<b>" . strtoupper($customer_name) . "</b>"; ?>. The Plan you choose is
                                                            <?php

                                                            $select = "SELECT * FROM startimes WHERE plan_code = '$bonquet'";
                                                            $result = mysqli_query($connection, $select);
                                                            while ($row = mysqli_fetch_assoc($result)) {
                                                                $plan = $row['plan_name'];
                                                                $_SESSION['plan_name'] = $plan;
                                                                echo "<b>" . strtoupper($plan) . "</b>";
                                                            }
                                                            ?>

                                                            Which is going to last for a <b>MONTH</b>
                                                            <b>
                                                                . This will cost you <?php echo "&#8358;" . $amount; ?>. Please if that is correct click on Proceed
                                                        </h5>

                                                    </center>
                                                </div>
                                                <div class="text-center">
                                                    <button type="submit" onclick="payWithPaystack()" class="btn btn-success"></i> Proceed</button>
                                                        ...
                                                        function payWithPaystack() {
                                                            var handler = PaystackPop.setup({
                                                                ...
                                                                callback: function(response) {
                                                                    ...
                                                                    window.location.href = 'startimeactivate.php?succesfullypaid=' + referenced;
                                                                },
                                                                onClose: function() {
